...
def gen_samepeople_Similarity(
    featsDict, peopleList, peopleName, feats, outfile_dist_info, fix_num=24
):
    """
        ...
        for i in range(0, len(index)):
            ...
            for j in range(pick_num):
                ...
                embedding_similarity = cos(
                    feats[index[i], :], feats[compared_index[j], :]
                )
    ...
    outfile_dist_info.write(
        f"Maximum similarity value: {np.max(same_similaritys)}" + "\n"
    )
    outfile_dist_info.write(
        f"Mean similarity value: {np.mean(same_similaritys)}" + "\n"
    )
    outfile_dist_info.write(
        f"Minute similarity value: {np.min(same_similaritys)}" + "\n"
    )
    outfile_dist_info.write(
        f"Std of similarity value: {np.std(same_similaritys)}" + "\n"
    )
    return pos_similarity_dist, len(same_similaritys)


def gen_diffpeople_Similarity(
    featsDict,
    peopleList,
    peopleName,
    feats,
    ...
    outfile_dist_info.write(
        f"Maximum similarity value: {np.max(diff_similaritys)}" + "\n"
    )
    outfile_dist_info.write(
        f"Mean similarity value: {np.mean(diff_similaritys)}" + "\n"
    )
    outfile_dist_info.write(
        f"Minute similarity value: {np.min(diff_similaritys)}" + "\n"
    )
    outfile_dist_info.write(
...
def norm_labels(data_root, outfile_wdistacne, outfile_result, id_score):
    ...
    with open(outfile_wdistacne, "r") as f:
        txtContent = f.readlines()
    imgpath = []
    quality_scores = []
    for i in tqdm(txtContent):
    ...
    quality_scores_select = (
        (quality_scores_select - np.min(quality_scores_select))
        / (np.max(quality_scores_select) - np.min(quality_scores_select))
        * 100
    )
...
if __name__ == "__main__":
    ...
    data_root = os.path.join("D:\\IQA\\da_fen_zhangjm", "newPdata4_jm")
    datalistFile = "D:\IQA\da_fen_zhangjm\quality\generate_pseudo_labels\DATA.label"
    # featsFile    = 'D:\IQA\da_fen_zhangjm\quality\\npy\npy.npy'
    featsFile = os.path.join("D:\\IQA\\da_fen_zhangjm\\quality", "npy", "npy.npy")
    ...
    for i in range(12):
        ...
        neg_similarity_dist = gen_diffpeople_Similarity(
            featsDict, peopleList, peopleName, feats, pos_pairs_num, outfile_dist_info
        )
